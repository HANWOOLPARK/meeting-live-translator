...
`dist\verbaradar-lite-YYYYMMDD.zip` 하나가 생성됩니다. 이 ZIP은 `.env`, API 키, 세션, 로그, PID, 가상환경, 모델, 테스트·PoC 자료를 제외하고 실행에 필요한 소스와 설치 스크립트만 포함합니다. 압축 내부의 SHA-256 manifest와 금지 경로·비밀 패턴 검사를 통과하지 못하면 ZIP 생성이 실패합니다. 받는 사람에게는 **이 ZIP 하나만** 전달하면 되며 설치 순서는 함께 포함된 `DISTRIBUTION_KO.md`에 설명되어 있습니다.
...
링크만으로는 회의 내용을 읽을 수 없습니다. 참석자는 자신의 이메일을 입력하고 이메일로
받은 6자리 일회용 인증번호를 10분 안에 확인해야 합니다. 인증번호는 5회 오류 시 잠기고,
재발송은 45초 간격과 이메일·IP별 횟수 제한을 적용합니다. 인증 성공 후에는 방별 HttpOnly,
Secure, SameSite 쿠키가 발급되어 새로고침해도 방이 끝나기 전까지 유지됩니다.
...
- 인증번호 도전 데이터: 코드 원문은 저장하지 않고 HMAC 해시만 보관, 최대 24시간 후 정리
- 링크별 접속 기록: 인증 이메일, 최초 인증·마지막 확인 시각, 입장·거절 이벤트를 30일 보관
  후 중계 정리 시점과 다음 로컬 앱 유지보수 시 자동 삭제. 원본 IP는 저장하지 않고 해시만 보관
...
인증 메일은 현재 Resend HTTPS API로 발송합니다. 공개 참석자에게 보내려면 Resend에서
본인 소유 도메인을 검증하고 `sending_access` 권한의 키를 만든 뒤 Sites production runtime에
다음 값을 secret/환경변수로 설정해야 합니다. 실제 값은 Git, `.openai/hosting.json`, 문서,
로컬 공유 URL에 넣지 않습니다.

```text
RESEND_API_KEY=<sending-only secret>
MLT_OTP_FROM_EMAIL=VerbaRadar <invite@verified.example>
MLT_OTP_SIGNING_SECRET=<별도의 32-byte 이상 무작위 secret>
```

키나 발신 주소가 없으면 새 공유방 생성은 `viewer_email_auth_not_configured`로 안전하게
거부되며 기존 로컬 전사·번역·Radar는 계속 동작합니다. 인증 메일 전송을 위해 참석자 이메일은
Resend로 전달되므로 입장 화면과 진행자 동의 영역에 이 외부 전송과 30일 감사 로그 보관을
표시합니다.
...
않습니다. 방 URL은 충분히 긴 무작위 ID이고 이메일 인증도 필요하지만 공개 채널에
재전송하지 않아야 합니다.
