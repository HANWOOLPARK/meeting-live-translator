...
- 링크별 접속 기록: 인증 이메일, 최초 인증·마지막 확인 시각과 입장 이벤트를 30일 보관
...
거부되며 기존 로컬 전사·번역·Radar는 계속 동작합니다. Google과 Supabase가 계정 소유 여부를
확인하고, `whykaigi_attendee_profiles`와 `whykaigi_room_access_logs`에는 RLS를 적용합니다.
입장 화면에는 이 외부 인증과 30일 감사 로그 보관을 표시합니다.
