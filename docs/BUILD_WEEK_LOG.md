...
- 외부 계정 조치: 과거 화면·대화에 노출된 Gemini 키와 공개 전환 전 relay secret은 저장소에 없지만, 제출 전 사용자가 각 Provider 콘솔에서 폐기·교체해야 한다. Devpost 영상·`/feedback` Session ID·최종 폼 제출은 사용자 계정에서 남은 작업이다.
