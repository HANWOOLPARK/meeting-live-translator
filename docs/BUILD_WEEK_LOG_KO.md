...
## 사용자가 직접 검토한 제작 계기 · 2026-07-21 KST

- 사용자가 직접 제공한 내용: 생활 일본어는 가능하지만 비즈니스 회의에서는 단어와 일부 맥락만 이해할 뿐, 회의의 전체 목적·논의 진행 정도·실제 결정 사항을 회의가 끝난 뒤 AI로 요약하기 전까지 알기 어려웠다. WhyKaigi는 이 이해를 회의 중에 얻기 위해 시작했으며, 길고 답답하게 느껴지던 회의가 이제는 직접 만든 앱이 실제 도움을 주는 것을 확인하는 기다려지는 시간이 됐다.
- 이름과 장기 방향: 사용자는 기존 BJT 비즈니스 일본어 학습 앱 Why BJT에서 WhyKaigi로 Why 시리즈를 확장했다. `WHY`는 사용자가 일본에 처음 왔을 때 문화 적응에 어려움을 겪으며 스스로 정한 `What Holds You Back?`의 줄임말이다. 장기적으로 일본에 사는 외국인이 일본어 공부, 미팅과 생활에 필요한 서비스를 한곳에서 찾는 Why 시리즈로 넓히고자 한다.
- AI 사용 경계와 검토: Codex는 사용자가 한국어로 제공한 사실만 영문 README와 Devpost `Inspiration` 구조에 맞게 번역·정리했다. 사용자는 게시 전에 완성된 영문 초안을 읽고 자신의 경험과 정확히 일치한다고 명시적으로 승인했다. 나머지 Devpost 섹션은 다음 개인 검토를 위해 변경하지 않았다.
- 두 번째 사용자 검토: 사용자는 Codex가 전통적인 구현 장애를 해결했으며 본인은 반복 실사용, 의도와 다른 결과 지적, 구체적인 기대값 제시를 담당했다고 설명했다. 또한 유료 API의 기대값을 알 수 없었던 점이 유일한 망설임이었고, 언어별 전사 확정 시점·번역 품질과 속도·선택적 로컬 Whisper 재검증을 가장 자랑스러운 제품 작업으로 꼽았다. Codex는 이 사실만 README 협업 문구와 Devpost `Challenges`, `Accomplishments` 구조로 정리했고 사용자는 게시 전에 영문 전체 초안을 직접 읽고 명시적으로 승인했다.
- 세 번째 사용자 검토: 사용자는 GPT-5.5에서 GPT-5.6으로 넘어오며 직접 느낀 변화, ChatGPT 브레인스토밍과 Codex 구현을 통해 스스로 가능하다고 판단하는 아이디어의 범위가 달라진 과정, 그리고 개발 지식의 부족보다 제품 판단·실사용 검증·사용자에게 도달하는 일이 더 중요해졌다는 생각을 제공했다. 다음 순서도 Build Week 중 동료 실사용, 번역 품질·속도 개선, 유료화 검증, 일본 거주 한국인에서 더 넓은 Why 시리즈 플랫폼으로 확장하는 것으로 직접 정했다. Codex는 이 의견만 영문 `What I learned`와 `What's next` 구조로 번역·정리했고 사용자는 전체 초안을 읽고 명시적으로 승인한 뒤 Devpost version 7에 저장했다.
- 네 번째 사용자 검토: 사용자는 WhyKaigi를 실시간 번역과 함께 회의의 중점·할 일을 판단하는 도구로 직접 정의했다. 첫인상은 빠르고 자연스러운 실시간 번역이어야 하며, 실시간을 표방하지만 어색하거나 지연되는 기존 도구와 달리 언어별 문맥에 맞는 전사 확정과 빠른 번역이 핵심이라고 설명했다. 또한 근거가 불명확한 추천과 달리 결정·할 일을 번역과 원문에서 직접 검증할 수 있다는 점을 두 번째 차별점으로 정했다. Codex는 이 우선순위만 README 첫 소개와 Devpost 태그라인으로 번역·정리했고 사용자는 게시 전 영문 초안을 명시적으로 승인했다.
...
## 사용자가 정한 WhyKaigi 정체성과 릴리스 강화 · 2026-07-21 KST

- 사용자 결정과 창작 경계: 사용자가 최종 제품명을 `WhyKaigi`로 직접 정했고, 본인이 만든 여러 앱에서 이미 사용하던 검은색 `Why?` 워드마크 원본을 제공했다. Codex는 이름이나 시각 콘셉트를 대신 창작하지 않고, 사용자가 제공한 표식을 정리·확대해 실행 환경과 제출 규격에 맞게 변환했다.
- 제품 표시명 변경: FastAPI 제목, Electron 앱·창 식별자, 메인 UI, 분리 자막·Radar 창, 공개 Viewer, OTP 메일 문구, 내보내기 제목, 패키지 메타데이터, 설치·시작·종료 메시지, Lite 배포명, 문서와 공개 Replay 표기를 `WhyKaigi`로 변경했다. 실행 화면은 동일한 512×512 PNG를 사용하며 Windows 다중 해상도 ICO, 3:2 Open Graph와 Devpost 자산도 함께 만들었다.
- 호환성 경계: `MLT_*` 환경변수, `mlt-*` 쿠키·저장소·Broadcast 키, API·relay 스키마, D1 binding, 세션 형식, 로컬 작업 폴더명과 기존 Sites 프로젝트 ID는 유지했다. 과거 요구사항과 VerbaRadar 개발 기록도 역사로 보존했다. 기존 세션·JSONL·모델·자격 증명·접속 로그는 수정하지 않았다.
- 회귀 방지: WhyKaigi 표시 계약, 세 실행용 512×512 PNG의 바이트 동일성, 구 자산 제거, 실행 소스의 구 표시명 부재, renderer cache token, Viewer production bundle의 로컬 Windows 사용자 경로 부재를 자동 검사한다. 이전 공개 HTML에 개발자 로컬 폰트 절대 경로가 포함된 원인이었던 `next/font`를 제거했다.
- 실제 검증: 전체 Python 회귀는 `369 passed, 3 skipped`; SKIP 3개는 기존의 명시적 live Provider 게이트다. Viewer ESLint, Vinext production build, Node 테스트 13개, JavaScript 구문 검사와 PowerShell parser 검사가 통과했다. 실제 Electron을 재시작해 FastAPI·번역 Worker·앱 준비·Loopback 장치·WebSocket 연결 상태와 새 `Why?` 표식 및 WhyKaigi 제목 표시를 확인했다.
- Lite 배포 근거: `whykaigi-lite-20260721.zip`은 115개 entry이며 추적된 비밀값, 세션, 모델, 가상환경, 실행 PID 파일, 구 브랜드 자산이 없다. SHA-256은 `CD2AE6361BFCF0A828ED12E65DABC7B63A0B30ECE014FE31D1854FDD316B86EC`다.
- GitHub 릴리스: 기존 공개 저장소를 `HANWOOLPARK/whykaigi`로 이름 변경해 이력과 권한을 보존했고 설명과 데모 홈페이지를 갱신했다. 공개 `v1.0.0-build-week` 릴리스 이름은 `WhyKaigi - Build Week Demo`이며 검증된 `whykaigi-lite-20260721.zip`만 포함한다. GitHub digest와 비로그인 재다운로드 SHA-256은 위 로컬 해시와 일치했다.
- Sites 배포: 검증된 Viewer tree를 기존 Sites 소스 저장소에 fast-forward로 반영하고 환경 revision 7을 사용하는 version 10을 공개했다. 사이트 제목과 OTP 발신자 표시명은 `WhyKaigi`이고 검증된 발신 주소, Resend key, relay secret, OTP secret, D1 DB, 접속 기록, 방과 공개 URL은 보존했다. 공개 랜딩과 `/demo`는 새 이름·아이콘을 표시했고 console 경고와 로컬 사용자 경로 노출이 없었다.
- 공개 미디어 확인: 배포된 데모 MP3는 HTTP 200, `audio/mpeg`, 919,721바이트이며 로컬 검증본 SHA-256 `96870E48C9AEDF776AC912EED27E37DED2A0D8E7F6B44E6F9A0C4D41740F089F`와 일치했다. Codex 내장 브라우저는 미디어를 완전히 buffer했지만 자체 재생 정책으로 소리를 거부했으므로 해당 브라우저의 오디오 재생 성공은 주장하지 않는다. 공개 바이트 동일성과 Replay·근거 테스트 13개 통과를 배포 근거로 사용한다.
- Devpost 갱신: 게시된 프로젝트 표시명과 설명 속 여섯 개 제품명을 `WhyKaigi`로 변경하고, 링크를 새 GitHub 저장소와 공개 Replay로 교체했으며 사용자가 만든 `Why?` 썸네일을 업로드했다. 데모 영상은 비워 두고 Build Week 최종 제출도 사용자의 촬영 전까지 진행하지 않았다. Devpost의 기존 `/software/verbaradar` slug와 Sites 기술 hostname은 호환 URL로 유지되지만 화면에 보이는 제품 제목과 이미지는 모두 WhyKaigi다.

## 공개 저장소와 Sites 프로덕션 배포 · 2026-07-20 17:18 KST

- 단일 루트 저장소의 236개 공개 후보를 커밋 직전 재검사했고 실제 API 키·로컬 사용자 경로·비공개 데모 세션 ID·5MiB 초과 파일은 각각 0건이었다. `.env`, `.share.env`, 세션, 로그, PID, 모델, 가상환경, `node_modules`, Sites 로컬 DB와 빌드 캐시는 Git 추적에서 제외됐다.
- 최초 검증 커밋 `5d2d1670dcb85da54cea152dbacb5d4ec4881a30`을 생성하고 `HANWOOLPARK/meeting-live-translator` 공개 저장소의 `main`으로 push했다. 중첩 Git 저장소는 없으며 `viewer-site` 소스도 동일 커밋에 포함됐다.
- Sites의 기존 원격 이력은 force-push로 덮어쓰지 않았다. 기존 HEAD를 부모로 두고 검증된 `viewer-site` subtree와 동일한 Git tree를 가진 fast-forward 커밋 `1c2b2824befb5e31be3bfa5b66e345398bba997b`를 push했다. 단기 소스 자격 증명은 명령 단위 HTTP 헤더에서만 사용했고 remote URL·Git config·파일에 저장하지 않았다.
- 플러그인 공식 package helper로 같은 소스의 `dist`, hosting metadata, D1 migration을 묶어 Sites 버전 5(52 files, 3,399,680 bytes)를 저장했다. 사용자가 승인한 제출 계획에 따라 접근 정책을 `public`으로 변경하고 해당 버전을 프로덕션에 배포했다.
- 비로그인·무키 HTTP 검증에서 `https://meeting-live-translator-viewer.bakbaul.chatgpt.site` 랜딩과 `/demo`는 모두 200이었고 Replay 진입·본문을 포함했다. 반면 secret 없는 `POST /api/rooms`는 401로 거부됐다. 배포 성공 후 로컬 뷰어 dev server를 종료했다.
- 외부 계정·relay 보안 조치: 공개 저장소에는 키가 없다. relay 생성 secret은 새 256비트 난수로 교체해 로컬 `.share.env`와 Sites secret 환경변수 revision 2에만 저장하고 버전 5를 재배포했다. 새 자격의 임시 방 E2E는 `201 생성 → 200 조회 → 200 삭제 → 410 삭제 확인`을 통과했고 방 내용과 host token을 출력하지 않았다. 과거 화면·대화에 노출된 Gemini API 키는 제출 전 사용자가 Google Provider 콘솔에서 폐기·교체해야 한다. Devpost 영상·`/feedback` Session ID·최종 폼 제출은 사용자 계정에서 남은 작업이다.
- GitHub Release `v1.0.0-build-week`를 draft·prerelease가 아닌 공개 릴리스로 생성하고 Lite ZIP을 첨부했다. 무인증 HTTP에서 저장소와 릴리스 페이지는 각각 200이었다. 원격 자산을 메모리로 다시 다운로드한 결과 355,155 bytes였고 SHA-256은 로컬 검증본과 같은 `FA0326E19D21962AE711801EAC648CED73A86FD0CEB16F6E45FE5C3D1FA5F9E5`였다.
- 데모 종료 후 활성 Context profile을 작업 전 사용자 profile로 복원했다. 프로세스 환경에만 적용했던 Luna 데모 서버를 `stop_all.bat`으로 정리하고 기존 `.env`를 변경하지 않은 채 `start_all.bat`로 재시작했다. 현재 FastAPI health는 `ok`, 로컬 번역 Worker는 `ready`이고 서버·Worker·desktop은 각자 프로젝트 PID로 실행 중이다.

## 참석자 링크의 Supabase Google 신원 인증 전환 · 2026-07-22 KST

- 사용자 문제와 결정: Resend 시험 모드에서는 계정 소유자에게만 인증 메일을 보낼 수
  있어 실제 초대 참석자의 신원을 확인하고 링크별 접속 기록을 남기는 목적에 맞지 않았다.
  사용자는 자체 이메일 OTP 대신 Supabase Google 인증을 사용하기로 결정했다.
- 인증 구현: 브라우저에서 Google OAuth를 수행하고, 반환된 Supabase access token은
  방별 exchange API에만 전달한다. 브라우저는 PKCE를 사용하고 현대식
  `sb_publishable_` key만 허용하므로 secret key나 기존 JWT key는 공개되지 않고 fail closed
  한다. 서버는 Supabase `/auth/v1/user`에 token을 직접 검증하며 클라이언트가 보내는 이메일
  문자열은 신뢰하지 않는다. 이미 Supabase에 로그인한 사용자도 링크를 열었다는 이유만으로
  자동 입장시키지 않고 **이 방에 입장하기**를 명시적으로 눌러야 한다. 검증 성공 후 기존의
  방별 Secure·HttpOnly·SameSite=Strict Viewer 세션을 발급한다. 폐기된 이메일 OTP
  request·verify endpoint는 기존 Resend 변수가 남아 있어도 항상 410을 반환하므로 대체
  Viewer 세션 발급 경로가 되지 않는다.
- 감사·보관·격리: Supabase는 신원 인증에만 사용한다. 방별 D1 접속 감사는 relay 서버만
  기록하는 권위 있는 로그이며 브라우저는 PostgREST로 profile이나 입장 이벤트를 쓰지 않는다.
  정리 SQL은 공유 프로젝트의 일반 `private` schema를 건드리지 않고 폐기된
  `whykaigi_attendee_profiles`, `whykaigi_room_access_logs`만 제거한다. 명시적 종료 시 relay는
  진행자에게 최종 감사 snapshot을 반환한 뒤 해당 방의 challenge, Viewer 세션, 접속 로그,
  신원 데이터와 중계 텍스트를 삭제한다. 유휴 만료나 방 최대 수명 만료는 snapshot 반환 없이
  같은 데이터를 삭제한다. 진행자 앱은 명시적 종료 때 받은 최종 링크별 감사 사본만 로컬에
  최대 30일 보관할 수 있다. 기존 세션 JSONL은 별도이며 변경하지 않는다.
- 배포: 제품 커밋 `fc3a8b2`의 Viewer tree를 정확히 분리해 Sites source 커밋
  `52482b2091c7dc7fea50da7f29c1203786afb02d`, Viewer version 12, 환경 revision 8로
  공개했다. Resend·OTP 환경변수는 제거된 상태이며 Supabase URL, 현대식 publishable key,
  별도 access signing secret만 설정했다. 기존 Why BJT·localhost redirect와 Viewer 방
  wildcard를 함께 유지했다. Version 12에는 PKCE, 방별 명시적 입장, relay 종료 정리,
  Supabase Auth 전용 구조와 anti-frame 보안 강화가 모두 포함된다.
- 최초 운영 검증: 시험방에서 Google 로그인→WhyKaigi 복귀→확인된 이메일 표시→새로고침
  유지→로그아웃을 확인했다. D1에는 `access_granted`, `viewer_entered`,
  `signed_out`이 남았다. 공유 종료 후 410을 확인했으며 검증용 인증 세션과 시험방을
  폐기했다. 최초 시험에서 사용한 Supabase 중앙 감사 구조는 이후 서버 소유 D1 감사와
  종료 시 즉시 정리 방식으로 교체했다.
- Version 12 운영 검증: 공개 인증 설정은 현대식 `sb_publishable_` key만 반환하고 secret
  필드는 노출하지 않았다. 폐기된 OTP endpoint는 cookie 없이 410을 반환했다. `/room/**`에는
  `frame-ancestors 'none'`, `X-Frame-Options: DENY`, private/no-store가 적용됐다. 일회용
  relay 방은 익명 조회와 token 없는 Supabase exchange를 401로 거부했고, 진행자 종료 때 빈
  최종 감사를 반환한 뒤 같은 방 조회를 410으로 처리했다.
- 회귀 검증: 전체 Python `369 passed, 3 skipped`, Viewer lint·production build,
  Node 테스트 `13 passed`. 익명 조회·exchange는 401, 필수 Supabase/signing 설정
  누락 시 새 방 생성은 503으로 안전하게 거부됐다. 계약 테스트에는 폐기 OTP endpoint,
  publishable key guard, PKCE, 명시적 방 입장, 서버 Google 신원 검증, D1 단독 감사,
  방 종료 정리와 Supabase profile·접속 로그 write 부재를 추가했다.
- 데이터 불변성: 최종 재검사에서 JSONL 137개의 경로·길이·파일별 SHA-256이 같았고,
  기준 집계 SHA-256은
  `22EC8102AD0CEA1BBADD229C59CE8FF6B11BEB3F7601ABEE3B8C00BE1F35545F`다.
- 절충: 현재 WhyKaigi는 기존 Why BJT Supabase 프로젝트의 Auth 사용자 풀과
  프로젝트 한도를 공유하지만 WhyKaigi 전용 profile·접속 로그 table은 더 이상 두지 않는다.
  독립 유료 서비스 출시 전에는 전용 Supabase 프로젝트로 분리하는 것을 권장한다.
- Build Week 기록: 최신 Devpost 기술 설명에서 오래된 이메일 OTP 설명을 실제 검증한
  Google/Supabase 흐름으로 교체하고 회귀 수치를 369로 갱신했다. 사용자가 검토한
  창업 이야기·제품 판단·데모 영상·제목·tagline·프로젝트 링크는 변경하지 않았으며
  제출 상태도 그대로 유지했다.
