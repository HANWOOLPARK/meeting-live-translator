...
- 인증 구현: 브라우저에서 Google OAuth를 수행하고, 반환된 Supabase access token은
  exchange API에만 전달한다. 서버가 Supabase Auth에 직접 token을 검증하므로
  클라이언트가 보내는 이메일 문자열은 신뢰하지 않는다. 검증 성공 후 기존의 방별
  Secure·HttpOnly·SameSite=Strict Viewer 세션을 발급하며 필수 설정이 없으면 fail closed 한다.
- 감사·격리: `whykaigi_attendee_profiles`, `whykaigi_room_access_logs`를 추가하고
  RLS를 활성화했다. `anon` table 접근은 없고 인증 사용자도 본인 row만 접근한다.
  D1에는 방별 운영 로그를, Supabase에는 확인된 Google 신원 기반의 30일 입장 감사를 남긴다.
- 배포: 제품 커밋 `91e2078`, Sites source 커밋
  ...
  redirect는 유지하면서 Viewer 방 wildcard를 허용 목록에 추가했다.
- 운영 검증: 시험방에서 Google 로그인→WhyKaigi 복귀→확인된 이메일 표시→새로고침
  유지→로그아웃을 확인했다. D1에는 `access_granted`, `viewer_entered`,
  `signed_out`이 남았고 Supabase에도 Google 신원 입장 감사가 기록됐다. 공유 종료 후
  410을 확인했으며 검증용 인증 세션과 시험방을 폐기했다.
- 회귀 검증: 전체 Python `369 passed, 3 skipped`, Viewer lint·production build,
  ...
  누락 시 새 방 생성은 503으로 안전하게 거부됐다. Supabase Security Advisor의 WhyKaigi
  보안 경고는 0건이었다.
...
- 절충: 현재 WhyKaigi는 기존 Why BJT Supabase 프로젝트의 Auth 사용자 풀과
  프로젝트 한도를 공유한다. 테이블과 정책은 격리했지만 독립 유료 서비스 출시 전에는
  전용 Supabase 프로젝트로 분리하는 것을 권장한다.
