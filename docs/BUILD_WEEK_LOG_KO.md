...
- 감사·보관·격리: Supabase는 신원 인증에만 사용한다. 방별 D1 접속 감사는 relay 서버만
  ...
  `whykaigi_attendee_profiles`, `whykaigi_room_access_logs`만 제거한다. 명시적 종료·유휴
  만료·방 최대 수명 만료 시 relay는 진행자에게 최종 감사 snapshot을 반환한 뒤 해당 방의
  challenge, Viewer 세션, 접속 로그, 신원 데이터와 중계 텍스트를 삭제한다. 진행자 앱은 이
  최종 링크별 감사 사본만 로컬에 최대 30일 보관할 수 있다. 기존 세션 JSONL은 별도이며
  변경하지 않는다.
- 배포: 제품 커밋 `91e2078`, Sites source 커밋
  `02e63d9d5d0726a3d36f790f677720bf5747f7d3`을 기반으로 Viewer version 11,
  환경 revision 8을 공개했다. Resend·OTP 환경변수를 제거하고 Supabase URL,
  publishable key, 별도 access signing secret을 설정했다. 기존 Why BJT와 localhost
  redirect는 유지하면서 Viewer 방 wildcard를 허용 목록에 추가했다. Version 11에서 Google
  운영 흐름을 먼저 확인했고, 위 PKCE·명시적 입장·relay 종료 정리·Auth 전용 구조는 다음
  Sites 공개를 위한 보안 강화 source다.
...
- Build Week 기록: Devpost version 11에서 오래된 이메일 OTP 설명을 실제 검증한
