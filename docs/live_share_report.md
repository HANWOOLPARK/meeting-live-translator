...
- `viewer-site/supabase/remove_whykaigi_client_audit.sql`: 폐기된 Supabase profile·입장 로그 table과
  정리 function만 제거하는 cleanup SQL. 새 table이나 client write 권한은 만들지 않음
...
- 명시적 종료는 진행자에게 최종 감사 snapshot을 반환한 뒤 `state_json`, 호스트 토큰 해시,
  방별 challenge·Viewer 세션·접속 로그·신원 데이터를 즉시 비우고 410을 반환한다.
- heartbeat가 15분 끊기거나 생성 후 8시간이 지나면 조회·업데이트 전에 같은 방별 정리를 한다.
...
- Supabase Auth가 Google 계정과 확인된 이메일을 검증한다. Viewer는 검증된 user ID·이메일로만
  방 세션을 만들고 relay 서버만 D1에 방별 운영 로그를 남긴다. Supabase에는 WhyKaigi profile이나
  입장 로그를 쓰지 않는다.
- 중계 서버의 인증 이메일·방 세션·입장 이벤트는 공유 종료나 만료 시 삭제한다. 진행자 앱은
  종료 직전 받은 링크별 감사 사본만 로컬에 최대 30일 보관할 수 있다.
...
- 공개 설정은 현대식 `sb_publishable_` key만 허용하므로 secret key나 기존 JWT key를
  클라이언트에 내보내지 않는다.
- 폐기된 이메일 OTP request·verify endpoint는 기존 Resend 변수가 남아 있어도 410만 반환하므로
  Google identity exchange 외의 경로로 Viewer 세션을 만들 수 없다.
- 브라우저 OAuth는 PKCE를 사용한다. 같은 origin에 기존 Supabase 세션이 있어도 링크 방문만으로
  방 세션을 만들지 않으며 사용자가 **이 방에 입장하기**를 눌러야 한다.
...
redirect URL을 허용해야 한다. 입장 전에 Google/Supabase 외부 인증, 중계 서버의 종료 시 삭제,
진행자 로컬 감사 사본의 최대 30일 보관을 고지한다. Supabase는 Auth에만 사용하며 참석자
profile·방 접속 감사용 공개 table이나 PostgREST write 경로를 두지 않는다.
...
- Supabase cleanup SQL은 폐기된 `whykaigi_attendee_profiles`,
  `whykaigi_room_access_logs`, 전용 prune function만 drop하며 공유 `private` schema나
  Why BJT object는 변경하지 않음. 신규 table·policy·insert grant는 0건
- Sites production version 11에서 Google 로그인→Viewer 복귀, 검증된 이메일 표시,
  새로고침 후 세션 유지, D1의 `access_granted`, `viewer_entered`, `signed_out` 감사 이벤트를
  확인했다. Hardened source는 version 12, environment revision 8로 공개했으며 현대식 공개
  key만 노출, 구 OTP 410, anti-frame header, 익명 조회·token 없는 exchange 401, 진행자 종료
  후 410을 운영 API에서 재검증했다. Supabase 중앙 입장 기록 없이 D1 단독 감사·종료 시 삭제
  구조다.
...
현재는 기존 Why BJT Supabase 프로젝트의 Auth 사용자 풀을 재사용하지만 WhyKaigi profile이나
접속 감사 table은 두지 않는다. 현재 개인 운영에는 충분하지만, WhyKaigi를 독립 유료 서비스로
출시하기 전에는 장애·quota·운영 권한을 분리하기 위해 전용 Supabase 프로젝트로 이전하는 것을
권장한다.
